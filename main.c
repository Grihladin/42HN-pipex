...
/*   Updated: 2024/11/24 18:50:47 by mratke           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int argc, char **argv, char **env)
{
	...
	file1_write_fd = open("file1", O_WRONLY | O_CREAT | O_TRUNC, 777);
	file1_read_fd = open("file1", O_RDONLY | O_CREAT | O_TRUNC, 777);
	if (pipe(pipe_fd) == -1)
	{
	...
	else if (process_id_2 == 0)
	{
		...
		dup2(file1_write_fd, STDOUT_FILENO);
		close(pipe_fd[0]);
		close(pipe_fd[1]);
		close(file1_write_fd);
		close(file1_read_fd);
